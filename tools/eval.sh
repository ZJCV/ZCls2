...
# Usage: bash tools/eval.sh <config-file> <gpus> <master-port>
...
if [ $# == 0 ]; then
  ...
  echo " e.g.2: CUDA_VISIBLE_DEVICES=0,1,2,3 bash tools/eval.sh configs/cfg.yaml 4"
  echo " e.g.2: CUDA_VISIBLE_DEVICES=0,1,2,3 bash tools/eval.sh configs/cfg.yaml 4 15231"
  exit 1
fi
